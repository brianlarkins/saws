 ...
 * ================================================
 *
 * This queue is split into three parts, as show in the diagram below: a part
 * that is for local access only (allowing for lock-free local interaction with
 * the queue), a part for shared access, and a reserved part that corresponds to
 * remote copies that are in-progress (ie deferred copies).
 *
 * Queue state:
 *
 * Head  - Always points to the element at the head of the local portion.
 *       - The head moves to the RIGHT.  Push and pop are both allowed wrt the
 * head. Tail  - Always points to the next available element at the tail of the
 * shared portion.
 *       - The tail moves to the RIGHT.  Only pop is allowed from the tail.
 * Split - Always points to the element at the tail of the local portion of the
 * queue. Vtail - Virtual tail: points to the element at the tail of the
 * reserved portion of the queue.  Everything between the head and the vtail is
 * free space. Itail - Tells us the collective progress of all transactions in
 * the reserved portion. When Itail == tail we can reclaim the reserved space.
 *
 * The ring buffer can in in several states:
 *
 * 1. Empty.  In this case we have nlocal == 0, tail == split, and vtail ==
 * itail == tail
 *
 * 2. No Wrap-around:
...
laws_t *laws_create(int elem_size, int max_size, tc_t *tc) {
  ...
  rb->avg_local_tasks_stolen = gtc_shmem_calloc(sizeof(int), 10000);
  rb->num_local_steals = gtc_shmem_calloc(sizeof(int), 10000);
  rb->avg_global_tasks_stolen = gtc_shmem_calloc(sizeof(int), 10000);
  rb->num_global_steals = gtc_shmem_calloc(sizeof(int), 10000);
...
int laws_local_isempty(laws_t *rb) { return rb->nlocal == 0; }

int laws_shared_isempty(laws_t *rb) { return rb->tail == rb->split; }

int laws_isempty(laws_t *rb) {
  return laws_local_isempty(rb) && laws_shared_isempty(rb);
}

int laws_local_size(laws_t *rb) { return rb->nlocal; }
...
void laws_lock(laws_t *rb, int proc) { synch_mutex_lock(&rb->lock, proc); }

int laws_trylock(laws_t *rb, int proc) {
  return synch_mutex_trylock(&rb->lock, proc);
}

void laws_unlock(laws_t *rb, int proc) { synch_mutex_unlock(&rb->lock, proc); }
  ...
  if (rb->max_size - (laws_local_size(rb) + laws_public_size(rb)) < n) {
    laws_lock(rb, rb->procid);
    {
      if (rb->max_size - laws_size(rb) < n) {
        ...
        printf(
            "laws: Error, not enough space in the queue to push %d elements\n",
            n);
        laws_print(rb);
        assert(0);
      }
...
int laws_reacquire(laws_t *rb) {
  ...
  laws_lock(rb, rb->procid);
  {
    ...
    assert(!laws_local_isempty(rb) ||
           (laws_isempty(rb) && laws_local_isempty(rb)));
  }
  laws_unlock(rb, rb->procid);
...
static inline void laws_push_n_head_impl(laws_t *rb, int proc, void *e, int n,
                                         int size) {
  int head, old_head;
  ...
  if (head > old_head || old_head == rb->max_size - 1) {
    memcpy(laws_elem_addr(rb, proc, (old_head + 1) % rb->max_size), e,
           n * size);
  }
  ...
  // This push wraps around, break it into two parts
    ...
    memcpy(laws_elem_addr(rb, proc, 0), laws_buff_elem_addr(rb, e, part_size),
           (n - part_size) * size);
  }
...
/* Pop up to N elements off the tail of the queue, putting the result into the
...
static inline int laws_pop_n_tail_impl(laws_t *myrb, int proc, int n, void *e,
                                       int steal_vol, int trylock) {
  laws_t trb;
  ...
  if (n > 0) {
    ...
    if ((&trb)->tail + (n - 1) < (&trb)->max_size) { // No need to wrap around
      ...
      shmem_getmem_nbi(e, laws_elem_addr(myrb, proc, (&trb)->tail),
                       n * (&trb)->elem_size,
                       proc); // Store n elems, starting at remote tail, in e
      ...
      shmem_getmem_nbi(laws_buff_elem_addr(&trb, e, 0),
                       laws_elem_addr(myrb, proc, (&trb)->tail),
                       part_size * (&trb)->elem_size, proc);

      shmem_getmem_nbi(laws_buff_elem_addr(&trb, e, part_size),
                       laws_elem_addr(myrb, proc, 0),
                       (n - part_size) * (&trb)->elem_size, proc);
    ...
    */
    ...
    // Accumulate itail_inc onto the victim's intermediate tail
    {
      ...
      // How much should we add to the itail?  If we caused a wraparound, we
      // need to also wrap itail.
