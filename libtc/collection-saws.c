 ...
 */
gtc_t gtc_create_saws(gtc_t gtc, int max_body_size, int shrb_size,
                      gtc_ldbal_cfg_t *cfg) {
  GTC_ENTRY();
  tc_t *tc;
  ...
  tc->shared_rb =
      saws_shrb_create(tc->max_body_size + sizeof(task_t), shrb_size, tc);
  tc->inbox = NULL;
...
int gtc_add_saws(gtc_t gtc, task_t *task, int proc) {
  ...
  if (proc == _c->rank) {
    ...
    saws_shrb_push_head(tc->shared_rb, _c->rank, task,
...
void gtc_print_stats_saws(gtc_t gtc) {
  ...
  if (!getenv("SCIOTO_DISABLE_STATS") &&
    ...
    printf(" %4d - saws-Q: nrelease %6lu, nreacquire %6lu, nreclaimed %6lu, "
...
void gtc_print_gstats_saws(gtc_t gtc) {
  ...
  eprintf("        : gets         %6lu (%6.2f/%3lu/%3lu) time "
          ...
          sumcounts[SAWSNumGets], sumcounts[SAWSNumGets] / (double)_c->size,
          mincounts[SAWSNumGets], maxcounts[SAWSNumGets],
          sumtimes[SAWSPopTailTime] / _c->size, mintimes[SAWSPopTailTime],
          maxtimes[SAWSPopTailTime], sumtimes[SAWSPerPopTailTime] / _c->size,
          mintimes[SAWSPerPopTailTime], maxtimes[SAWSPerPopTailTime]);

  eprintf("        :   get_buf    %6lu (%6.2f/%3lu/%3lu\n",
          sumcounts[SAWSGetCalls], sumcounts[SAWSGetCalls] / (double)_c->size,
          mincounts[SAWSGetCalls], maxcounts[SAWSGetCalls]);

  eprintf("        :   get_meta   %6lu (%6.2f/%3lu/%3lu) time "
          "%6.2fms/%6.2fms/%6.2fms per %6.2fms/%6.2fms/%6.2fms\n",
          sumcounts[SAWSNumMeta], sumcounts[SAWSNumMeta] / (double)_c->size,
          mincounts[SAWSNumMeta], maxcounts[SAWSNumMeta],
          sumtimes[SAWSGetMetaTime] / _c->size, mintimes[SAWSGetMetaTime],
          maxtimes[SAWSGetMetaTime], sumtimes[SAWSPerGetMetaTime] / _c->size,
          mintimes[SAWSPerGetMetaTime], maxtimes[SAWSPerGetMetaTime]);

  eprintf("        :   localget   %6lu (%6.2f/%3lu/%3lu)\n",
          sumcounts[SAWSGetLocalCalls],
          sumcounts[SAWSGetLocalCalls] / (double)_c->size,
  ...
  eprintf("        :   fails lock %6lu (%6.2f/%3lu/%3lu)\n",
          sumcounts[SAWSStealFailsLocked],
          sumcounts[SAWSStealFailsLocked] / (double)_c->size,
          mincounts[SAWSStealFailsLocked], maxcounts[SAWSStealFailsLocked]);
  eprintf("        :   fails un   %6lu (%6.2f/%3lu/%3lu)\n",
          sumcounts[SAWSStealFailsUnlocked],
          sumcounts[SAWSStealFailsUnlocked] / (double)_c->size,
          mincounts[SAWSStealFailsUnlocked], maxcounts[SAWSStealFailsUnlocked]);
  eprintf("        :   fails ab   %6lu (%6.2f/%3lu/%3lu)\n",
          sumcounts[SAWSAbortedSteals],
          sumcounts[SAWSAbortedSteals] / (double)_c->size,
  ...
  eprintf("        : progress   %6.2f/%3lu/%3lu time %6.2fus/%6.2fus/%6.2fus "
          ...
          sumcounts[SAWSProgressCalls] / (double)_c->size,
          mincounts[SAWSProgressCalls], maxcounts[SAWSProgressCalls],
          sumtimes[SAWSProgressTime] / _c->size, mintimes[SAWSProgressTime],
          maxtimes[SAWSProgressTime], sumtimes[SAWSPerProgressTime] / _c->size,
          mintimes[SAWSPerProgressTime], maxtimes[SAWSPerProgressTime]);
  eprintf("        : reclaim    %6.2f/%3lu/%3lu time %6.2fus/%6.2fus/%6.2fus "
          "per %6.2fus/%6.2fus/%6.2fus\n",
          sumcounts[SAWSReclaimCalls] / (double)_c->size,
          mincounts[SAWSReclaimCalls], maxcounts[SAWSReclaimCalls],
          sumtimes[SAWSReclaimTime] / _c->size, mintimes[SAWSReclaimTime],
          maxtimes[SAWSReclaimTime], sumtimes[SAWSPerReclaimTime] / _c->size,
          mintimes[SAWSPerReclaimTime], maxtimes[SAWSPerReclaimTime]);
  eprintf("        : ensure     %6.2f/%3lu/%3lu time %6.2fus/%6.2fus/%6.2fus "
          "per %6.2fus/%6.2fus/%6.2fus\n",
          sumcounts[SAWSEnsureCalls] / (double)_c->size,
          mincounts[SAWSEnsureCalls], maxcounts[SAWSEnsureCalls],
          sumtimes[SAWSEnsureTime] / _c->size, mintimes[SAWSEnsureTime],
          maxtimes[SAWSEnsureTime], sumtimes[SAWSPerEnsureTime] / _c->size,
          mintimes[SAWSPerEnsureTime], maxtimes[SAWSPerEnsureTime]);
  eprintf("        : reacquire  %6.2f/%3lu/%3lu time %6.2fms/%6.2fms/%6.2fms "
          "per %6.2fms/%6.2fms/%6.2fms\n",
          sumcounts[SAWSReacquireCalls] / (double)_c->size,
          mincounts[SAWSReacquireCalls], maxcounts[SAWSReacquireCalls],
          sumtimes[SAWSReacquireTime] / _c->size, mintimes[SAWSReacquireTime],
          maxtimes[SAWSReacquireTime],
          sumtimes[SAWSPerReacquireTime] / _c->size,
          mintimes[SAWSPerReacquireTime], maxtimes[SAWSPerReacquireTime]);
  eprintf("        : release    %6.2f/%3lu/%3lu time %6.2fus/%6.2fus/%6.2fus "
          "per %6.2fus/%6.2fus/%6.2fus\n",
          sumcounts[SAWSReleaseCalls] / (double)_c->size,
          mincounts[SAWSReleaseCalls], maxcounts[SAWSReleaseCalls],
          sumtimes[SAWSReleaseTime] / _c->size, mintimes[SAWSReleaseTime],
          maxtimes[SAWSReleaseTime], sumtimes[SAWSPerReleaseTime] / _c->size,
          mintimes[SAWSPerReleaseTime], maxtimes[SAWSPerReleaseTime]);

  eprintf("&&&  %6.2f %6.2f ", sumtimes[SAWSPopTailTime] / _c->size,
          sumtimes[SAWSReacquireTime] / _c->size);
