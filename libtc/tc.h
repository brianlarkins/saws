...
#else
#warning                                                                       \
    "NO_SEATBELTS Defined -- Building without safety check and performance analysis"
#define assert(X)
#endif
...
// Enable/Disable different classes of debugging statements by OR-ing these
// flags together to form DEBUGLEVEL
#define DBGINIT 1
#define DBGPROCESS 2
#define DBGGET 4
#define DBGTD 8
...
// #define GTC_DEFAULT_DEBUGLEVEL DBGERR | DBGWARN | DBGINIT // | DBGTD | DBGGET
// | DBGSHRB | DBGMEM | DBGPROCESS | DBGTASK | DBGSYNCH

// line added by me, dingus supreme
#define GTC_DEFAULT_DEBUGLEVEL DBGERR | DBGWARN | DBGINIT
...
#define __GTC_MAX_STEAL_SIZE                                                   \
  500000                         // max # of tasks to steal when doing work sp
...
typedef struct {
  int stealing_enabled;  /* Is stealing enabled?  If not, the load balance is
                            static with pushing */
  ...
  int max_steal_retries; /* Max number of retries before we abort.  Set low to
                            limit contention. -1 means infinity */
  int max_steal_attempts_local; /* Max number of lock attempts before we "retry"
                                   a local target. */
  int max_steal_attempts_remote; /* Max number of lock attempts before we
                                    "retry" a remote target. */
  int chunk_size;                /* Size of a steal when using STEAL_CHUNK */
  int local_search_factor; /* Percent of steal attempts (0-100) that should
                              target intra-node targets */
} gtc_ldbal_cfg_t;
...
struct task_class_desc_s {
  ...
  void *pool; // Allocation pool to avoid alloc cost XXX may want to make this a
              // linked list or do slab allocations
};
...
struct tc_counters_s {
  ...
  tc_counter_t total_steals;     // Number of successful steals
  tc_counter_t num_local_steals; // number of successful intranode steals
  tc_counter_t steals_at_run;    // number of successful intranode steals
  tc_counter_t runs;             // number of successful intranode steals
  tc_counter_t last_run;         // number of successful intranode steals
  tc_counter_t this_run;         // number of successful intranode steals
  double avg_steals_per_run;     // number of successful intranode steals
  tc_counter_t
      failed_steals_locked; // # steal attempts that failed after locking
  tc_counter_t
      failed_steals_unlocked; // # steal attempts that failed before locking
  tc_counter_t
      aborted_steals; // # steal attempts that were aborted due to contention
  tc_counter_t aborted_targets; // # targets that were aborted due to contention
  tc_counter_t
      dispersion_attempts_locked; // failed_steals_locked during dispersion
  tc_counter_t
      dispersion_attempts_unlocked; // failed_steals_unlocked during dispersion
...
enum gtc_status_e { GtcStatusOK, GtcStatusError };
...
// task.c
task_class_t gtc_task_class_register(int body_size,
                                     void (*cb_execute)(gtc_t gtc,
                                                        task_t *descriptor));
...
#define gtc_task_body_size(TSK)                                                \
  gtc_task_class_lookup((TSK)->task_class)->body_size
...
// collection-sdc.c
gtc_t gtc_create_sdc(gtc_t gtc, int max_body_size, int shrb_size,
                     gtc_ldbal_cfg_t *ldbal_cfg);
...
gtc_t gtc_create_saws(gtc_t gtc, int max_body_size, int shrb_size,
                      gtc_ldbal_cfg_t *cfg);
gtc_t gtc_create_sh(gtc_t gtc, int max_body_size, int shrb_size,
                    gtc_ldbal_cfg_t *cfg);
 ...
 */
static inline long gtc_mythread(void) { return _c->rank; }

/**
 *  gtc_nthreads - gets count of processors in computation
 *  @returns size of calling process
 */
static inline long gtc_nthreads(void) { return _c->size; }

/**
 * gtc_barrier - convenience function for OpenSHMEM barrier
 */
static inline void gtc_barrier(void) { shmem_barrier_all(); }
...
static inline uint64_t gtc_get_tsctime() { return __rdtsc(); }

#ifdef GTC_USE_INTERNAL_TIMERS

#ifdef GTC_USE_TSC_TIMERS
#define TC_CPU_HZ _c->tsc_cpu_hz *(double)1e6
#define TC_INIT_ATIMER(TMR)                                                    \
  do {                                                                         \
    TMR.total = 0;                                                             \
    TMR.temp = 0;                                                              \
    TMR.last = 0;                                                              \
...
#define TC_ADD_TIMER(TC, TMR, ATMR)                                            \
  do {                                                                         \
...
#define TC_READ_ATIMER_NSEC(TMR)                                               \
  (double)(TC_READ_ATIMER(TMR) / (TC_CPU_HZ)) * (double)1e9
#define TC_READ_ATIMER_USEC(TMR)                                               \
  (double)(TC_READ_ATIMER(TMR) / (TC_CPU_HZ)) * (double)1e6
#define TC_READ_ATIMER_MSEC(TMR)                                               \
  (double)(TC_READ_ATIMER(TMR) / (TC_CPU_HZ)) * (double)1000.0
...
#define TC_READ_ATIMER(TMR)                                                    \
  (double)((1000000000L * (TMR.total.tv_sec)) + TMR.total.tv_nsec)
