...
gtc_t gtc_create(int max_body_size, int chunk_size, int shrb_size, gtc_ldbal_cfg_t *ldbal_cfg, gtc_qtype_t qtype) {
  ...
  _sanity->tcs[gtc] = shmem_malloc(sizeof(tc_t));
...
void gtc_print_gstats(gtc_t gtc) {
  ...
  for (int i=0; i<ncounts; i++) {
    counts[i] = mincounts[i] = maxcounts[i] = sumcounts[i] = 0.0;
  }
...
void gtc_print_stats(gtc_t gtc) {
  ...
  eprintf("        : imbalance  %6.2fms/%6.2fms/%6.2fms  termination attempts: %d\n",
      sumtimes[ImbalanceTime]/_c->size, mintimes[ImbalanceTime], maxtimes[ImbalanceTime], tc->td->num_attempts);
