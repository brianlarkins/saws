 ...
 *
 * @param max_body_size  IN Max size of a task descriptor's body in bytes for
 * this tc.  Any task   that is inserted must be of this size.  For tasks with
 * descriptors smaller than task_size they should be placed in buffers of
 * task_size in length.
 * @param chunk_size IN Number of tasks to be transferred as the result of a
 * steal operation.
 *
 * @return           pointer to task collection handle
 */
gtc_t gtc_create(int max_body_size, int chunk_size, int shrb_size,
                 gtc_ldbal_cfg_t *ldbal_cfg, gtc_qtype_t qtype) {
  gtc_t gtc;
  tc_t *tc;
  int localalloc = 0;
  ...
  if (!gtc_is_initialized) {
    gtc_is_initialized =
        -1; // signify that we are initializing sciotwo implicitly
    gtc_init();
  }
  ...
  if (ldbal_cfg->steal_method == STEAL_CHUNK) {
    tc->steal_buf =
        gtc_malloc(ldbal_cfg->chunk_size * (sizeof(task_t) + max_body_size));
  } else {
    tc->steal_buf =
        gtc_malloc((shrb_size / 2) * (sizeof(task_t) + max_body_size));
  }
  tc->qtype = qtype;
...
void gtc_print_config(gtc_t gtc) {
  ...
  if (tc->ldbal_cfg.stealing_enabled) {
    idx += snprintf(msg + idx, size - idx, ", Target selection: %s",
                    target_methods[tc->ldbal_cfg.target_selection]);

    if (tc->ldbal_cfg.steal_method == STEAL_CHUNK)
      idx += snprintf(msg + idx, size - idx, ", Steal method: %s (%d)",
                      steal_methods[tc->ldbal_cfg.steal_method],
                      tc->ldbal_cfg.chunk_size);
    else
      idx += snprintf(msg + idx, size - idx, ", Steal method: %s",
                      steal_methods[tc->ldbal_cfg.steal_method]);
...
int gtc_add(gtc_t gtc, task_t *task, int proc) {
  GTC_ENTRY();
  tc_t *tc = gtc_lookup(gtc);
  GTC_EXIT(tc->cb.add(gtc, task, proc));
}
...
task_t *gtc_task_inplace_create_and_add(gtc_t gtc, task_class_t tclass) {
  GTC_ENTRY();
  tc_t *tc = gtc_lookup(gtc);
  GTC_EXIT(tc->cb.inplace_create_and_add(gtc, tclass));
}
...
void gtc_task_inplace_create_and_add_finish(gtc_t gtc, task_t *t) {
  GTC_ENTRY();
  tc_t *tc = gtc_lookup(gtc);
  GTC_EXIT(tc->cb.inplace_ca_finish(gtc, t));
}
...
int gtc_tasks_avail(gtc_t gtc) {
  GTC_ENTRY();
  tc_t *tc = gtc_lookup(gtc);
  GTC_EXIT(tc->cb.tasks_avail(gtc));
}
...
/**
 * Change the state of the external work flag.  Note, this flag should be
 * stable. Once it transition from true to false it should stay that way in
 * order to ensure proper termination detection.
 *
...
int gtc_get_local_buf(gtc_t gtc, int priority, task_t *buf) {
  ...
  tc_t *tc = gtc_lookup(gtc);
  UNUSED(priority);
  GTC_EXIT(tc->rcb.pop_head(tc->shared_rb, _c->rank, buf));
}

/**
 * gtc_steal_tail -- Attempt to steal a chunk of tasks from the given target's
 * tail.
 *
...
int gtc_steal_tail(gtc_t gtc, int target) {
  ...
  stealsize = tc->rcb.pop_n_tail(tc->shared_rb, target, req_stealsize,
                                 tc->steal_buf, tc->ldbal_cfg.steal_method);
  TC_STOP_ATIMER(temp);
  ...
  if (stealsize > 0) {
    gtc_lprintf(DBGGET,
                "\tthread %d: steal try: %d got: %d tasks from thread %d\n",
    ...
    // gtc_lprintf(DBGGET, "\tthread %d: failed steal got no tasks from thread
    // %d\n", _c->rank, target);
  }
...
/**
 * gtc_try_steal_tail -- Attempt to steal a chunk of tasks from the given
 * target's tail.
 *
 * @param  target        Process ID of target
 * @param  req_stealsize Amount of work to steal.  If this number is negative,
 * perform work splitting and get up to abs(req_stealsize) tasks.
...
int gtc_try_steal_tail(gtc_t gtc, int target) {
  ...
  if (tc->qtype == GtcQueueSAWS)
    stealsize =
        tc->cb.try_pop_n_tail(tc->shrb, target, req_stealsize, tc->steal_buf,
                              tc->ldbal_cfg.steal_method);
  else
    stealsize =
        tc->cb.try_pop_n_tail(tc->shared_rb, target, req_stealsize,
                              tc->steal_buf, tc->ldbal_cfg.steal_method);
...
void gtc_print_gstats(gtc_t gtc) {
  ...
  times[AcquireTime] = tc->ct.num_steals ? (TC_READ_TIMER_SEC(tc, passive) -
                                            TC_READ_TIMER_SEC(tc, imbalance)) /
                                               tc->ct.num_steals
                                         : 0.0;
  ...
  if (ext_stats_enabled == NULL) {
    eprintf(
        "Total  : stolen %3lu, steals %3lu Average: stolen %3lu, steals %3lu\n",
        sumcounts[TasksStolen], sumcounts[NumSteals],
        sumcounts[TasksStolen] / _c->size, sumcounts[NumSteals] / _c->size);
    eprintf("Time   : worst dispersion %8.5fms, worst imbalance %8.5fms, best "
            "imbalance %8.5fms,"
            " avg acquire %8.5fms, avg search %8.5fs (%5.2f %%)\n",
            maxtimes[DispersionTime] * 1000.0, maxtimes[ImbalanceTime] * 1000.0,
  ...
  nanosleep(&sleep, NULL); // lag a little to make sure that final results
                           // appear after all other stats i/o is done.

  eprintf("SCIOTO : Process time %.5f s, passive time %.5f s (%.2f%%), %lu "
          "tasks completed, %.2f tasks/sec (%.2f tasks/sec/PE)\n",
...
void gtc_print_stats(gtc_t gtc) {
  ...
  times[AcquireTime] = tc->ct.num_steals ? (TC_READ_TIMER_MSEC(tc, passive) -
                                            TC_READ_TIMER_MSEC(tc, imbalance)) /
                                               tc->ct.num_steals
                                         : 0.0;
  ...
  counts[TasksStolen] = tc->ct.tasks_stolen;
  counts[NumSteals] = tc->ct.num_steals;
  counts[DispersionAttempts] =
      tc->ct.dispersion_attempts_locked + tc->ct.dispersion_attempts_unlocked;
  ...
  // printf("%g\n", avg_tasks_per_steal);
  // 9 char to colon
  eprintf("process: %.5f : %.5f size: %d\n", sumtimes[ProcessTime],
          times[ProcessTime], _c->size);
  eprintf("SCIOTWO : queue: %s \n", gtc_queue_name(gtc));
  eprintf("        : process time %.5f s, passive time %.5f s (%.2f%%), search "
          "time %.5f ms\n",
          sumtimes[ProcessTime] / _c->size, sumtimes[PassiveTime] / _c->size,
          (sumtimes[PassiveTime] / sumtimes[ProcessTime]) * 100.0,
          sumtimes[SearchTime] / _c->size);
  ...
  eprintf("        : dispersion %6.2fms/%6.2fms/%6.2fms attempts %6.2f "
          "(%6.2f/%6.2f/%6.2f)\n",
          sumtimes[DispersionTime] / _c->size, mintimes[DispersionTime],
          maxtimes[DispersionTime], sumcounts[DispersionAttempts],
          sumcounts[DispersionAttempts] / _c->size,
          mincounts[DispersionAttempts], maxcounts[DispersionAttempts]);

  eprintf("        : imbalance  %6.2fms/%6.2fms/%6.2fms  termination attempts: "
          "%d\n",
          sumtimes[ImbalanceTime] / _c->size, mintimes[ImbalanceTime],
          maxtimes[ImbalanceTime], tc->td->num_attempts);

  tc->cb.print_gstats(gtc);

  // this is the graph data.
  //  num_processes, average_time, passive_time, search_time,
  //  average_dispersion_time, dispersion_attempts, tasks/sec, tasks/sec/pe,
  //  passive_pct
  eprintf("c  %lu      %.5f %.5f %.5f %6.2f %6.2f %6.2f %.2f %d %4.2f\n",
          _c->size, (sumtimes[ProcessTime] / _c->size),
          (sumtimes[PassiveTime] / _c->size), (sumtimes[SearchTime] / _c->size),
          (sumtimes[DispersionTime] / _c->size), 0.0,
          (sumcounts[TasksCompleted] / sumtimes[ProcessTime]),
          (sumcounts[TasksCompleted] / (sumtimes[ProcessTime] / _c->size)),
          tc->td->num_attempts,
          (sumtimes[PassiveTime] / sumtimes[ProcessTime]) * 100.0);
...
void gtc_print_my_stats(gtc_t gtc) {
  ...
  if (!pernode_stats_disabled) {
    printf(" %4d - Tasks: completed %3lu, spawned %3lu, stolen %3lu\n"
           ...
           " %4d -  Time: process %8.5fs = active %8.5fs + passive %8.5fs, "
           "passive count=%lu (%8.5fms avg), searching %8.5fs (%5.2f %%)\n"
           " %4d -      : dispersion %8.5fms, disp. steal fails -- unlocked "
           "%3lu, locked %3lu\n"
           " %4d -      : avg acquire %8.5fms, imbalance %8.5fms\n"
           " %4d -      : timers: %8.5fms %8.5fms %8.5fms %8.5f %8.5fms\n",
           _c->rank, tc->ct.tasks_completed, tc->ct.tasks_spawned,
           tc->ct.tasks_stolen, _c->rank, tc->ct.num_steals,
           tc->ct.failed_steals_unlocked + tc->ct.failed_steals_locked,
           tc->ct.aborted_steals, _c->rank, TC_READ_TIMER_SEC(tc, process),
           TC_READ_TIMER_SEC(tc, process) - TC_READ_TIMER_SEC(tc, passive),
           TC_READ_TIMER_SEC(tc, passive), tc->ct.passive_count,
           TC_READ_TIMER_MSEC(tc, passive) / tc->ct.passive_count,
           TC_READ_TIMER_SEC(tc, search),
           (TC_READ_TIMER(tc, search) / (double)TC_READ_TIMER(tc, process)) *
               100.0,
           _c->rank, TC_READ_TIMER_MSEC(tc, dispersion),
           tc->ct.dispersion_attempts_unlocked,
           tc->ct.dispersion_attempts_locked, _c->rank,
           (tc->ct.num_steals > 0) ? avg_acquire_time_ms : 0.6,
           TC_READ_TIMER_MSEC(tc, imbalance), _c->rank,
           TC_READ_TIMER_MSEC(tc, t[0]), TC_READ_TIMER_MSEC(tc, t[1]),
           TC_READ_TIMER_MSEC(tc, t[2]), TC_READ_TIMER_MSEC(tc, t[3]),
...
unsigned long gtc_stats_tasks_completed(gtc_t gtc) {
  GTC_ENTRY();
  tc_t *tc = gtc_lookup(gtc);
  GTC_EXIT(tc->ct.tasks_completed);
}
