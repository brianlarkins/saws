...
gtc_t gtc_create_sdc(gtc_t gtc, int max_body_size, int shrb_size,
  ...
  tc->shared_rb =
      sdc_shrb_create(tc->max_body_size + sizeof(task_t), shrb_size, tc);
...
int gtc_add_sdc(gtc_t gtc, task_t *task, int proc) {
  ...
  if (proc == _c->rank) {
    ...
    sdc_shrb_push_head(tc->shared_rb, _c->rank, task,
...
void gtc_print_stats_sdc(gtc_t gtc) {
  ...
  if (!getenv("SCIOTO_DISABLE_STATS") &&
    ...
    printf(" %4d - SDC-Q: nrelease %6lu, nreacquire %6lu, nreclaimed %6lu, "
...
void gtc_print_gstats_sdc(gtc_t gtc) {
  ...
  counts[SDCNumTasksStolen] = tc->ct.tasks_stolen;
  ...
  percsteals =
      ((double)sumcounts[SDCNumLocalSteals] / (double)sumcounts[SDCNumSteals]) *
      100;
  avg_tasks_per_steal =
      (double)sumcounts[SDCNumTasksStolen] / (double)sumcounts[SDCNumSteals];
  ...
  eprintf("        : gets         %6lu (%6.2f/%3lu/%3lu) time "
          ...
          sumcounts[SDCNumGets], sumcounts[SDCNumGets] / (double)_c->size,
          mincounts[SDCNumGets], maxcounts[SDCNumGets],
          sumtimes[SDCPopTailTime] / _c->size, mintimes[SDCPopTailTime],
          maxtimes[SDCPopTailTime], sumtimes[SDCPerPopTailTime] / _c->size,
          mintimes[SDCPerPopTailTime], maxtimes[SDCPerPopTailTime]);

  eprintf("        :   get_buf    %6lu (%6.2f/%3lu/%3lu\n",
          sumcounts[SDCGetCalls], sumcounts[SDCGetCalls] / (double)_c->size,
  ...
  eprintf("        :   get_meta   %6lu (%6.2f/%3lu/%3lu) time "
          ...
          maxtimes[SDCGetMetaTime], sumtimes[SDCPerGetMetaTime] / _c->size,
          mintimes[SDCPerGetMetaTime], maxtimes[SDCPerGetMetaTime]);

  eprintf("        :   localget   %6lu (%6.2f/%3lu/%3lu)\n",
          sumcounts[SDCGetLocalCalls],
          sumcounts[SDCGetLocalCalls] / (double)_c->size,
  ...
  eprintf("        :   local steals     %6lu (%6.2f/%3lu/%3lu) perc. of steals "
          "overall %6g\n",
          sumcounts[SDCNumLocalSteals],
          sumcounts[SDCNumLocalSteals] / (double)_c->size,
          mincounts[SDCNumLocalSteals], maxcounts[SDCNumLocalSteals],
          percsteals);
  eprintf("        :   get_tasks   time %6.2fms/%6.2fms/%6.2fms per "
          "%6.2fus/%6.2fus/%6.2fus; avg tasks per steal: %6.2f tasks\n",

          sumtimes[SDCStealTime] / _c->size, mintimes[SDCStealTime],
          maxtimes[SDCStealTime], sumtimes[SDCPerStealTime] / _c->size,
          mintimes[SDCPerStealTime], maxtimes[SDCPerStealTime],
          avg_tasks_per_steal);
  eprintf("        :   fails lock %6lu (%6.2f/%3lu/%3lu)\n",
          sumcounts[SDCStealFailsLocked],
          sumcounts[SDCStealFailsLocked] / (double)_c->size,
          mincounts[SDCStealFailsLocked], maxcounts[SDCStealFailsLocked]);
  eprintf("        :   fails un   %6lu (%6.2f/%3lu/%3lu)\n",
          sumcounts[SDCStealFailsUnlocked],
          sumcounts[SDCStealFailsUnlocked] / (double)_c->size,
          mincounts[SDCStealFailsUnlocked], maxcounts[SDCStealFailsUnlocked]);
  eprintf("        :   fails ab   %6lu (%6.2f/%3lu/%3lu)\n",
          sumcounts[SDCAbortedSteals],
          sumcounts[SDCAbortedSteals] / (double)_c->size,
  ...
  eprintf("        : progress   %6.2f/%3lu/%3lu time %6.2fus/%6.2fus/%6.2fus "
          ...
          sumcounts[SDCProgressCalls] / (double)_c->size,
          mincounts[SDCProgressCalls], maxcounts[SDCProgressCalls],
          sumtimes[SDCProgressTime] / _c->size, mintimes[SDCProgressTime],
          maxtimes[SDCProgressTime], sumtimes[SDCPerProgressTime] / _c->size,
          mintimes[SDCPerProgressTime], maxtimes[SDCPerProgressTime]);
  eprintf("        : reclaim    %6.2f/%3lu/%3lu time %6.2fus/%6.2fus/%6.2fus "
          "per %6.2fus/%6.2fus/%6.2fus\n",
          sumcounts[SDCReclaimCalls] / (double)_c->size,
          mincounts[SDCReclaimCalls], maxcounts[SDCReclaimCalls],
          sumtimes[SDCReclaimTime] / _c->size, mintimes[SDCReclaimTime],
          maxtimes[SDCReclaimTime], sumtimes[SDCPerReclaimTime] / _c->size,
          mintimes[SDCPerReclaimTime], maxtimes[SDCPerReclaimTime]);
  eprintf("        : ensure     %6.2f/%3lu/%3lu time %6.2fus/%6.2fus/%6.2fus "
          "per %6.2fus/%6.2fus/%6.2fus\n",
          sumcounts[SDCEnsureCalls] / (double)_c->size,
          mincounts[SDCEnsureCalls], maxcounts[SDCEnsureCalls],
          sumtimes[SDCEnsureTime] / _c->size, mintimes[SDCEnsureTime],
  ...
  eprintf("        : reacquire  %6.2f/%3lu/%3lu time %6.2fms/%6.2fms/%6.2fms "
          ...
          maxtimes[SDCReacquireTime], sumtimes[SDCPerReacquireTime] / _c->size,
          mintimes[SDCPerReacquireTime], maxtimes[SDCPerReacquireTime]);
  eprintf("        : release    %6.2f/%3lu/%3lu time %6.2fus/%6.2fus/%6.2fus "
          "per %6.2fus/%6.2fus/%6.2fus\n",
          sumcounts[SDCReleaseCalls] / (double)_c->size,
          mincounts[SDCReleaseCalls], maxcounts[SDCReleaseCalls],
          sumtimes[SDCReleaseTime] / _c->size, mintimes[SDCReleaseTime],
          maxtimes[SDCReleaseTime], sumtimes[SDCPerReleaseTime] / _c->size,
          mintimes[SDCPerReleaseTime], maxtimes[SDCPerReleaseTime]);

  eprintf("&&&  %6.2f %6.2f ", sumtimes[SDCPopTailTime] / _c->size,
          sumtimes[SDCReacquireTime] / _c->size);
