...
int sdc_shrb_local_isempty(sdc_shrb_t *rb) { return rb->nlocal == 0; }

int sdc_shrb_shared_isempty(sdc_shrb_t *rb) { return rb->tail == rb->split; }

int sdc_shrb_isempty(sdc_shrb_t *rb) {
  return sdc_shrb_local_isempty(rb) && sdc_shrb_shared_isempty(rb);
}

int sdc_shrb_local_size(sdc_shrb_t *rb) { return rb->nlocal; }
  ...
  if (rb->max_size - (sdc_shrb_local_size(rb) + sdc_shrb_public_size(rb)) < n) {
    sdc_shrb_lock(rb, rb->procid);
    {
      if (rb->max_size - sdc_shrb_size(rb) < n) {
        ...
        printf("SDC_SHRB: Error, not enough space in the queue to push %d "
               "elements\n",
               n);
        sdc_shrb_print(rb);
        assert(0);
...
int sdc_shrb_reacquire(sdc_shrb_t *rb) {
  ...
  sdc_shrb_lock(rb, rb->procid);
  {
    ...
    assert(!sdc_shrb_local_isempty(rb) ||
           (sdc_shrb_isempty(rb) && sdc_shrb_local_isempty(rb)));
  }
  sdc_shrb_unlock(rb, rb->procid);
...
static inline void sdc_shrb_push_n_head_impl(sdc_shrb_t *rb, int proc, void *e,
                                             int n, int size) {
  int head, old_head;
  ...
  if (head > old_head || old_head == rb->max_size - 1) {
    memcpy(sdc_shrb_elem_addr(rb, proc, (old_head + 1) % rb->max_size), e,
           n * size);
  }
  ...
  // This push wraps around, break it into two parts
    ...
    memcpy(sdc_shrb_elem_addr(rb, proc, 0),
           sdc_shrb_buff_elem_addr(rb, e, part_size), (n - part_size) * size);
  }
 ...
 * and should also be allocated with sdc_shrb_malloc().
...
static inline int sdc_shrb_pop_n_tail_impl(sdc_shrb_t *myrb, int proc, int n,
                                           void *e, int steal_vol,
                                           int trylock) {
  sdc_shrb_t trb;
  ...
  if (n > 0) {
    ...
    if ((&trb)->tail + (n - 1) < (&trb)->max_size) { // No need to wrap around
      ...
      shmem_getmem_nbi(e, sdc_shrb_elem_addr(myrb, proc, (&trb)->tail),
                       n * (&trb)->elem_size,
      ...
      shmem_getmem_nbi(sdc_shrb_buff_elem_addr(&trb, e, 0),
                       sdc_shrb_elem_addr(myrb, proc, (&trb)->tail),
                       part_size * (&trb)->elem_size, proc);

      shmem_getmem_nbi(sdc_shrb_buff_elem_addr(&trb, e, part_size),
                       sdc_shrb_elem_addr(myrb, proc, 0),
